...
export default function Chat() {
    ...
    const { loading, sendMessage, sessions, loadSessions, loadSession, startNewSession, currentSessionId } = useChat();
    ...
    const handleAction = async (action) => {
        console.log(`[${new Date().toISOString()}] Button clicked:`, action.label);
        console.log("Processing started for action:", action);

        try {
            console.log("Adding local message 'Processing'...");
            addLocalMessage('user', `🔄 Processing ${action.label}...`);
            ...
            if (action.type === 'reschedule' || action.type === 'UPDATE_TASK') {
                ...
                await planService.fetchDraft(action.payload.context, 'daily');
                console.log("planService.fetchDraft done. Fetching active plan...");
                await fetchActivePlan();
                console.log("fetchActivePlan done. Adding success message...");
                addLocalMessage('assistant', "✅ Routine generated! Check your dashboard.");
